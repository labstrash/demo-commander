...
--     original migration's sign-off note — confirmed still wanted: reserved
--     for future on-demand API validation-rejection rows; the batch path
--     itself never writes a row with these null, since every row it writes
--     comes from a fully-assembled message.
